...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"joker_{userid}_{pack}_anim"
    elif is_video:
        return f"joker_{userid}_{pack}_vid"
    return f"joker_{userid}_{pack}"
...
async def jokerpkg(_):
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمه بنجاح ✓ \nالحزمه  → [اضغط هنا](https://t.me/addstickers/{joker_Jep.set.short_name})**")
