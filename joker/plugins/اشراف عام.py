...
async def gben(joker):
    dc = razan = joker
    i = 0
    sender = await dc.get_sender()
    me = await joker.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await joker.get_chat()
    if joker.is_private:
        user = joker.chat
        rank = joker.pattern_match.group(1)
    else:
        joker.chat.title
    try:
        user, rank = await get_full_user(joker)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await joker.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await joker.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
