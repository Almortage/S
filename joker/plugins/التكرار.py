...
async def spam_function(event, joker, l313l, sleeptimem, sleeptimet, DelaySpam=False):
    ...
    if len(l313l) == 2:
        ...
        for _ in range(counter):
            ...
            if event.reply_to_msg_id:
                await joker.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and joker.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            joker = await event.client.send_file(
                event.chat_id, joker, caption=joker.text
            )
            await _catutils.unsavegif(event, joker)
        ...
        if BOTLOG:
            ...
            joker = await event.client.send_file(BOTLOG_CHATID, joker)
            await _catutils.unsavegif(event, joker)
        return
    elif event.reply_to_msg_id and joker.text:
        spam_message = joker.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    joker = await event.get_reply_message()
    ...
    await spam_function(event, joker, l313l, sleeptimem, sleeptimet)
